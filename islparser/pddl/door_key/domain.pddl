...
(define (domain hri)
    ...
    (:predicates
        ...
        ; doors
        (locked ?l1 - location ?l2 - location)
    ...
    (:action move
        ...
        :precondition (and
            (robot_at ?from)
            (not (or (locked ?from ?to) (locked ?to ?from)))
        )
        :effect (and
            (not (robot_at ?from))
            (robot_at ?to)
    ...
    (:action unlock
        :parameters (?location1 - location ?location2 - location)
        :precondition (and
            (robot_at ?location1)
            (or (locked ?location1 ?location2) (locked ?location2 ?location1))
        )
        :effect (and
